...
void MainWindow::setupSimpleDemo(QCustomPlot * customPlot)
{
	...
	customPlot->setInteractions(QCP::iRangeDrag | QCP::
				    iRangeZoom | QCP::iSelectPlottables);
}
...
void MainWindow::setupSincScatterDemo(QCustomPlot * customPlot)
{
	...
	customPlot->
	    graph(3)->setScatterStyle(QCPScatterStyle
				      (QCPScatterStyle::ssCross, 4));
	...
	for (int i = 0; i < shapes.size(); ++i) {
		...
		customPlot->graph()->
		    setName(QCPScatterStyle::
			    ...
			    (QCPScatterStyle::staticMetaObject.indexOfEnumerator
			     ("ScatterShape")).valueToKey(shapes.at(i)));
		...
		if (shapes.at(i) != QCPScatterStyle::ssCustom) {
			...
			for (int i = 0; i < 3; ++i)
				customScatterPath.cubicTo(qCos
							  (2 * M_PI * i / 3.0) *
							  9,
							  qSin(2 * M_PI * i /
							       3.0) * 9,
...
void MainWindow::setupLineStyleDemo(QCustomPlot * customPlot)
{
	...
	for (int i = QCPGraph::lsNone; i <= QCPGraph::lsImpulse; ++i) {
		...
		customPlot->graph()->setPen(pen);
		customPlot->graph()->
		    setName(lineNames.at(i - QCPGraph::lsNone));
		...
		for (int j = 0; j < 15; ++j) {
			...
			y[j] =
			    7 * sin(x[j]) / x[j] - (i - QCPGraph::lsNone) * 5 +
...
void MainWindow::setupScatterPixmapDemo(QCustomPlot * customPlot)
{
	...
	customPlot->
	    graph()->setScatterStyle(QCPScatterStyle(QPixmap("./sun.png")));
	// set graph name, will show up in legend next to icon:
	customPlot->
	    graph()->setName("Data from Photovoltaic\nenergy barometer 2011");
	// set data:
...
void MainWindow::setupMultiAxisDemo(QCustomPlot * customPlot)
{
	...
	customPlot->
	    graph(4)->setScatterStyle(QCPScatterStyle
				      (QCPScatterStyle::ssCircle, 4));
	customPlot->
	    graph(4)->setName("Some random data around\na quadratic function");
	...
	for (int i = 0; i < 250; ++i)	// data for graphs 2, 3 and 4
	{
		...
		y3[i] = cos(x3[i]) * 10;
		y4[i] =
		    0.01 * x4[i] * x4[i] + 1.5 * (rand() / (double)RAND_MAX -
						  0.5) + 1.5 * M_PI;
	}
	...
	piTicks << 0 << 0.5 * M_PI << M_PI << 1.5 * M_PI << 2 * M_PI << 2.5 *
	    M_PI << 3 * M_PI;
	piLabels << "0" << QString::
	    fromUtf8("½π") << QString::fromUtf8("π") << QString::
	    fromUtf8("1½π") << QString::fromUtf8("2π") << QString::
	    fromUtf8("2½π") << QString::fromUtf8("3π");
...
void MainWindow::setupBarChartDemo(QCustomPlot * customPlot)
{
	...
	fossilData << 0.86 * 10.5 << 0.83 * 5.5 << 0.84 * 5.5 << 0.52 *
	    5.8 << 0.89 * 5.2 << 0.90 * 4.2 << 0.67 * 11.2;
	nuclearData << 0.08 * 10.5 << 0.12 * 5.5 << 0.12 * 5.5 << 0.40 *
	    5.8 << 0.09 * 5.2 << 0.00 * 4.2 << 0.07 * 11.2;
	regenData << 0.06 * 10.5 << 0.05 * 5.5 << 0.04 * 5.5 << 0.06 *
	    5.8 << 0.02 * 5.2 << 0.07 * 4.2 << 0.25 * 11.2;
...
void MainWindow::setupStatisticalDemo(QCustomPlot * customPlot)
{
	...
	QCPStatisticalBox *sample1 =
	    new QCPStatisticalBox(customPlot->xAxis, customPlot->yAxis);
	QCPStatisticalBox *sample2 =
	    new QCPStatisticalBox(customPlot->xAxis, customPlot->yAxis);
	QCPStatisticalBox *sample3 =
	    new QCPStatisticalBox(customPlot->xAxis, customPlot->yAxis);
	...
	sample2->setOutliers(QVector <
			     double >() << 0.7 << 0.39 << 0.45 << 6.2 << 5.84);
	...
	customPlot->xAxis->setTickVectorLabels(QVector < QString >
					       () << "Sample 1" << "Sample 2" <<
					       "Control Group");
...
void MainWindow::setupItemDemo(QCustomPlot * customPlot)
{
#if QT_VERSION < QT_VERSION_CHECK(4, 7, 0)
	QMessageBox::critical(this, "",
			      "You're using Qt < 4.7, the animation of the item demo needs functions that are available with Qt 4.7 to work properly");
#endif
	...
	wavePacketText->setPositionAlignment(Qt::AlignBottom | Qt::
					     AlignHCenter);
	...
	phaseTracerArrow->setTail(QCPLineEnding
				  (QCPLineEnding::esBar,
				   (phaseTracerText->bottom->pixelPoint().y() -
				    phaseTracerText->top->pixelPoint().y()) *
				   0.85));
	...
	groupTracerArrow->setTail(QCPLineEnding
				  (QCPLineEnding::esBar,
				   (groupTracerText->bottom->pixelPoint().y() -
				    groupTracerText->top->pixelPoint().y()) *
				   0.85));
...
void MainWindow::setupStyledDemo(QCustomPlot * customPlot)
{
	...
	for (int i = 0; i < x1.size(); ++i) {
		...
		y1[i] =
		    qCos(x1[i] * 0.8 +
			 qSin(x1[i] * 0.16 + 1.0)) * qSin(x1[i] * 0.54) + 1.4;
	}
	for (int i = 0; i < x2.size(); ++i) {
		x2[i] = i / (double)x2.size() * 10;
		y2[i] =
		    qCos(x2[i] * 0.85 +
			 qSin(x2[i] * 0.165 + 1.1)) * qSin(x2[i] * 0.50) + 1.7;
	}
	...
	customPlot->xAxis->
	    grid()->setPen(QPen(QColor(140, 140, 140), 1, Qt::DotLine));
	customPlot->yAxis->
	    grid()->setPen(QPen(QColor(140, 140, 140), 1, Qt::DotLine));
	customPlot->xAxis->
	    grid()->setSubGridPen(QPen(QColor(80, 80, 80), 1, Qt::DotLine));
	customPlot->yAxis->
	    grid()->setSubGridPen(QPen(QColor(80, 80, 80), 1, Qt::DotLine));
	customPlot->xAxis->grid()->setSubGridVisible(true);
	customPlot->yAxis->grid()->setSubGridVisible(true);
...
void MainWindow::setupAdvancedAxesDemo(QCustomPlot * customPlot)
{
	...
	mainGraph1->setScatterStyle(QCPScatterStyle
				    (QCPScatterStyle::ssCircle, QPen(Qt::black),
				     QBrush(Qt::white), 6));
	...
	QCPGraph *graph2 =
	    customPlot->addGraph(subRectLeft->axis(QCPAxis::atBottom),
